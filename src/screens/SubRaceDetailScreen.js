...
const SubRaceDetailScreen = ({ route, navigation }) => {
	...
	const getResult = async id => {
		const response = await axios.get(
			`https://www.dnd5eapi.co/api/subraces/${id}`
		);
						...
						<View style={styles.container}>
							...
							{result.languages.length > 0 ? (
								<>
									...
									<Text style={styles.infoFont}>{result.languages}</Text>
								</>
							) : null}

							<Text style={styles.category}>Base Race</Text>
							<Text style={styles.infoFont}>
								{Object.values(result.race.name)}
							</Text>

							{result.racial_traits.name ? (
								<>
									<Text style={styles.category}>Racial Traits</Text>
									<Text style={styles.infoFont}>
										{Object.values(result.racial_traits.name)}
									</Text>
								</>
							) : null}
